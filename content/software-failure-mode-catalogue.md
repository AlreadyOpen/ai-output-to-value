...
It is not a maturity score, vulnerability ranking, prevalence ranking, severity ranking, or claim that every project needs every control. **Entries are displayed alphanumerically by failure-mode title. Their position carries no prevalence, severity, likelihood, priority, or recommended remediation-order meaning.** Use the failure modes that are material to the **decision and intended use**.

The canonical machine-readable catalogue is [`data/failure-modes.yml`](../data/failure-modes.yml). The public build also exposes an alphanumerically sorted view through the publication API.
...
## Catalogue at a glance — alphanumeric order
...
| **Concurrency/state-transition race** | Sequential tests pass; concurrent calls create impossible or stale state. | **Deliverable** | Concurrency/property tests and invariant checks |
| **Environment and configuration gap** | Local defaults, credentials or permissions differ from the target environment. | **Deliverable** | Representative-environment test, configuration review |
...
| **Hidden manual or expert step** | Original builder silently compensates from memory or local state. | **Capability** | Runbook rehearsal, handover test, dependency map |
| **Observability and alerting blindness** | Service runs while directly watched, but failure is invisible in normal operation. | **Capability** | Monitoring/alert test, synthetic check |
| **Retry duplicates a side effect** | One successful call works; ambiguous retry creates a second effect. | **Deliverable** | Idempotency, retry and reconciliation tests |
| **Rollback and recovery gap** | Forward deployment works; bad change or partial failure cannot be recovered. | **Capability** | Rollback, restore or failover rehearsal |
| **Scale, cost, or latency cliff** | Demo volume hides queueing, context growth, retries, rate limits or cost. | **Capability** | Load/capacity/rate-limit test, cost per acceptable outcome |
| **Schema migration preserves syntax but loses meaning** | SQL executes, but real historical data or invariants are damaged. | **Deliverable** | Representative-data rehearsal, invariants, rollback/forward-fix test |
| **Dependency and supply-chain integrity gap** | Dependency resolves, but provenance/integrity/update path is unverified. | **Deliverable** | Locked/reproducible build, provenance/dependency review |
...
## Observability and alerting blindness
...
## Retry duplicates a side effect
...
## Schema migration preserves syntax but loses meaning
...
Those sources support specific engineering concerns. They do **not** establish the six-claim framework, rank these failure modes by prevalence, severity, likelihood, or priority, or prove that a particular control is proportionate for a particular project.
